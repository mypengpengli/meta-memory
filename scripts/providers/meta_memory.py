...
class MetaMemoryProvider:
    ...
    def initialize(self, *, subject_id: str, session_id: str, profile_id: str, workspace_id: str, agent_context: str) -> None:
        ...
        internal = ensure_session(self.root, subject_id=subject_id, session_id=session_id, profile_id=profile_id, workspace_id=workspace_id)
        snapshot = freeze_hot_snapshot(self.root, internal_session_id=internal, subject_id=subject_id, profile_id=profile_id, workspace_id=workspace_id)
    ...
    def prefetch(self, query: str, *, session_id: str, token_budget: int) -> MemoryContext:
        ...
        result = retrieve(Namespace(
            ...
            workspace_id=self.workspace_id, valid_at=None, no_chunks=False,
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str, messages: list[dict]) -> None:
        first = insert_raw_event(self.root, subject_id=self.subject_id, subject_name="Unknown", session_id=session_id, source_type="conversation-user", content=user_content, profile_id=self.profile_id, workspace_id=self.workspace_id)
        second = insert_raw_event(self.root, subject_id=self.subject_id, subject_name="Unknown", session_id=session_id, source_type="conversation-assistant", content=assistant_content, profile_id=self.profile_id, workspace_id=self.workspace_id)
    ...
    def on_session_end(self, messages: list[dict]) -> None:
        close_session(self.root, self.session_id, subject_id=self.subject_id, profile_id=self.profile_id, workspace_id=self.workspace_id); enqueue_review(self.root, subject_id=self.subject_id, session_id=self.session_id, event_start_id=0, event_end_id=0, trigger_type="session_end", workspace_id=self.workspace_id)
