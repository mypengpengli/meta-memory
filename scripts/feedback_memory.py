...
def record_feedback(root, *, claim_id: str, feedback_type: str, source: str = "user", note: str = "", retrieval_uid: str = "") -> dict[str, object]:
    ...
    conn=open_db(root)
    claim=conn.execute("SELECT id,subject_id,subject_name,memory_kind,domain,topic,title,content,confidence,importance,durability,sensitivity,predicate,subject_text,object_text,profile_id,workspace_id,origin_agent_id,visibility_scope,owner_agent_id FROM claims WHERE id=?",(claim_id,)).fetchone()
    if not claim: conn.close(); raise ValueError("Claim not found")
    conn.close()
    raw_event_id = None
    if note.strip():
        raw_event_id = int(insert_raw_event(root, subject_id=str(claim[1]), subject_name=str(claim[2] or "Unknown"), source_type="user-memory-feedback", content=note.strip(), allow_duplicate=True, profile_id=str(claim[15] or "default"), workspace_id=str(claim[16] or "default"), origin_agent_id=str(claim[17] or ""), visibility_scope=str(claim[18] or "workspace"))["raw_event_id"])
    conn=open_db(root)
    ...
    if feedback_type in {"incorrect","outdated"}:
        ...
        plan={"plan_id":f"feedback:{claim_id}:{feedback_type}:{raw_event_id or 'clarify'}","action":action if raw_event_id else f"REVIEW_{action}","subject_id":str(claim[1]),"target_claim_id":claim_id,"source_event_ids":sources,"memory_kind":str(claim[3]),"domain":str(claim[4]),"topic":str(claim[5]),"title":str(claim[6]),"content":note.strip(),"confidence":float(claim[8]),"importance":float(claim[9]),"durability":float(claim[10]),"sensitivity":str(claim[11]),"predicate":str(claim[12]),"subject_text":str(claim[13]),"object_text":str(claim[14]),"profile_id":str(claim[15] or "default"),"workspace_id":str(claim[16] or "default"),"origin_agent_id":str(claim[17] or ""),"visibility_scope":str(claim[18] or "workspace"),"owner_agent_id":str(claim[19] or ""),"requires_review":True,"risk_reason":f"feedback:{feedback_type}"}
        proposal_id=stage_memory_proposal(conn,plan,origin="feedback",reason=f"feedback_{feedback_type}")
        conn.execute("UPDATE claims SET needs_reextract=1 WHERE id=?", (claim_id,))
        if not raw_event_id:
